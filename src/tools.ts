  ...
  return defineTool({
    ...
    parameters: {
      ...
      items: { type: 'array', required: true, items: { type: 'object', additionalProperties: false, properties: {
        type: { type: 'string', required: true, enum: ['goal', 'evidence', 'history-conflict', 'current-implementation', 'scope', 'semantics', 'completeness', 'acceptance', 'risk'] }, severity: { type: 'string', required: true, enum: ['blocking', 'major', 'minor'] }, statement: { type: 'string', required: true }, epistemicStatus: { type: 'string', required: true, enum: ['FACT', 'INFERENCE', 'ASSUMPTION', 'TO_VERIFY'] }, impact: { type: 'string', required: true }, question: { type: 'string', required: true, description: '只问一个可直接回答的问题，使用简洁中文。' }, recommendedOptions: { type: 'array', required: true, items: { type: 'string' }, description: '默认三个、最多四个互有取舍的中文选项；每项只表达一个决定，建议不超过 45 个汉字。' }, sourceAnchors: { type: 'array', required: true, description: '直接从当前 Spec 原文复制 1-3 段精确短句；问题来自缺失内容时可为空数组。', items: { type: 'object', additionalProperties: false, properties: { quote: { type: 'string', required: true }, heading: { type: 'string' } } } }, affectedSections: { type: 'array', required: true, items: { type: 'string' } }, affectedAcceptanceIds: { type: 'array', required: true, items: { type: 'string' } }, ownerRole: { type: 'string', required: true, enum: ['product', 'engineering'] }, status: { type: 'string', required: true, enum: ['open', 'joint-review', 'non-blocking-verify'] }, evidence: { type: 'array', required: true, items: { type: 'object', additionalProperties: false, properties: { statement: { type: 'string', required: true }, source: { type: 'string', required: true }, version: { type: 'string' }, accessible: { type: 'boolean', required: true } } } },
      } } },
    },
    ...
    async execute(args) {
      for (const item of args.items) {
        ...
        if (options.length > 4) throw new Error('each review item allows at most four recommended options')
        if (options.some(option => [...option].length > 60)) throw new Error('recommended options must be concise (60 characters maximum)')
        if ([...item.question.trim()].length > 80) throw new Error('review questions must be concise (80 characters maximum)')
...
export function specProposePatchTool(engine: CollaborationEngine) {
  return defineTool({
    name: 'spec_propose_patch', description: 'Propose a complete, plain-language Chinese Markdown Spec. Keep audit metadata in review records, not in the document. This creates a reviewable candidate only; it never writes Git.',
    ...
    async execute(args) { if (/\b(?:FACT|INFERENCE|ASSUMPTION|TO_VERIFY)\b/.test(args.proposedMarkdown)) throw new Error('candidate Spec must not contain internal evidence status enums'); if (/^\s*本文为.{0,30}候选\s*patch/i.test(args.proposedMarkdown)) throw new Error('candidate Spec must start with its document title, not a candidate disclaimer'); if (/\b(?:Given|When|Then)\b/.test(args.proposedMarkdown)) throw new Error('candidate Spec acceptance criteria must use 前提 / 操作 / 预期结果'); engine.submitPatch({ requirementId: args.requirementId, baseCommit: args.baseCommit, reviewKind: args.reviewKind as ReviewKind, ai: ai(args.nickname, args.role as ParticipantRole, args.sessionId), proposedMarkdown: args.proposedMarkdown, summary: args.summary, affectedReviewItemIds: args.affectedReviewItemIds, affectedSections: args.affectedSections, affectedAcceptanceIds: args.affectedAcceptanceIds }); return { content: `candidate patch proposed for ${args.baseCommit}` } },
  })
}
