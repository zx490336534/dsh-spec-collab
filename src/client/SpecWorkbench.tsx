...
export function SpecWorkbench({ api }: { api: SpecApi }) {
  ...
  const [sideTab, setSideTab] = useState<SideTab>('review')
  ...
  const productFirstRunning = selected?.aiRuns.some(run => run.kind === 'product-first' && run.commit === selected.currentCommit && ['queued', 'running'].includes(run.status)) ?? false
  const currentMyItems = selected ? myItems.filter(item => item.requirementId === selected.id) : []
  const tabCounts: Record<SideTab, number> = selected ? {
    review: selected.reviewItems.filter(item => !['resolved', 'invalidated'].includes(item.status)).length,
    discussion: selected.comments.filter(item => item.status === 'open').length,
    patches: selected.patches.filter(item => item.status === 'pending').length,
    decisions: selected.decisions.filter(item => item.status === 'active').length,
    items: currentMyItems.length,
    versions: 0,
    ready: selected.readiness.filter(item => !item.passed).length,
  } : { review: 0, discussion: 0, patches: 0, decisions: 0, items: 0, versions: 0, ready: 0 }
  const renderSideTab = (tab: SideTab) => <button key={tab} className={sideTab === tab ? css.activeMode : ''} onClick={() => { setSideTab(tab); setMobilePane('collab') }}>{tabLabel[tab]}{tabCounts[tab] > 0 && <span className={css.tabBadge}>{tabCounts[tab]}</span>}</button>
  ...
  const act = async (factory: (actor: ParticipantSnapshot) => CollaborationAction): Promise<boolean> => {
    ...
    try {
      ...
      if (!result.ok) {
        ...
        setNotice(result.error); return false
      }
  ...
  const create = async (): Promise<void> => {
    ...
    if (ok) { setCreateOpen(false); setCreateForm({ title: '', rawRequirement: '', sources: '', workspaceId: '' }); setSideTab('review') }
  }
  ...
  return <div className={css.workbench}>
    <header className={css.header}>
      <div className={css.titleBlock}><strong>{selected?.title ?? 'AI 需求澄清与 Spec 共创'}</strong>{selected && <span>{stageLabel(selected.stage)} · {selected.workspaceId ? reviewWorkspaces.find(workspace => workspace.workspaceId === selected.workspaceId)?.title ?? selected.workspaceId : '未绑定工作区'} · {selected.currentCommit.slice(0, 10)} · {selected.reviewItems.filter(item => item.severity === 'blocking' && item.status !== 'resolved').length} 阻塞</span>}</div>
      <div className={css.headerActions}>
        ...
        {selected?.stage === 'joint-review' && <><button disabled={!selected.workspaceId} onClick={() => void requestReview('engineering-precheck')}>研发智能预审</button><button onClick={() => void confirm(participant.role)}>以{participant.role === 'product' ? '产品' : '研发'}确认</button><button className={css.execute} onClick={() => selected && void act(actor => ({ kind: 'ready.generate', participant: actor, requirementId: selected.id }))}>生成就绪规范</button></>}
        <button className={css.identity} onClick={() => setIdentityOpen(true)}>{registered ? `${participant.nickname} · ${participant.role === 'product' ? '产品' : '研发'}` : '匿名只读'}</button>
      </div>
    </header>
    <div className={notice ? css.notice : css.noticeHidden}>{notice}<button aria-label="关闭" onClick={() => setNotice('')}>×</button></div>
    <div className={css.mobileViewSwitch}><button className={mobilePane === 'spec' ? css.activeMode : ''} onClick={() => setMobilePane('spec')}>规范正文</button><button className={mobilePane === 'collab' ? css.activeMode : ''} onClick={() => setMobilePane('collab')}>协作处理{selected ? ` · ${selected.actionItems.filter(item => item.status === 'pending').length}` : ''}</button></div>
    <div className={`${css.grid} ${mobilePane === 'spec' ? css.mobileSpec : css.mobileCollab}`} style={{ '--documents-width': documentsOpen ? `${documentsWidth}px` : '0px', '--documents-handle': documentsOpen ? '6px' : '0px', '--review-width': reviewOpen ? `${reviewWidth}px` : '0px', '--review-handle': reviewOpen ? '6px' : '0px' } as CSSProperties}>
      <aside className={`${css.documents} ${documentsOpen ? '' : css.paneClosed}`}><div className={css.paneHead}><strong>{showArchived ? '已归档需求' : '需求'}</strong><div className={css.paneActions}><button onClick={() => { setShowArchived(value => !value); setSelectedId(undefined); setManageMenuId(undefined) }}>{showArchived ? '进行中' : '已归档'}</button>{!showArchived && <button title="输入原始需求" onClick={() => { setCreateOpen(true); void api.reviewWorkspaces().then(setReviewWorkspaces, error => setNotice(error instanceof Error ? error.message : String(error))) }}>＋</button>}<button className={css.iconButton} aria-label="收起需求栏" title="收起需求栏" onClick={() => setDocumentsOpen(false)}>‹</button></div></div><nav>{visibleRequirements.map(item => <div className={css.requirementRow} key={item.id}><button className={`${css.documentButton} ${item.id === selected?.id ? css.selectedDoc : ''}`} onClick={() => setSelectedId(item.id)}><span>{item.title}</span><small>{item.archivedAt ? `归档于 ${new Date(item.archivedAt).toLocaleDateString()}` : `${stageLabel(item.stage)} · ${item.actionItems.filter(task => task.status === 'pending' && task.blocking).length} 待办`}</small></button><button className={css.rowMenuButton} aria-label={`${item.title} 更多操作`} title="更多操作" onClick={() => { setSelectedId(item.id); setManageMenuId(current => current === item.id ? undefined : item.id) }}>…</button>{manageMenuId === item.id && <div className={css.requirementMenu}><button onClick={() => { setManageTargetId(item.id); setRenameTitle(item.title); setRenameOpen(true); setManageMenuId(undefined) }}>重命名</button><button onClick={() => void archiveRequirement(item)}>{item.archivedAt ? '取消归档' : '归档需求'}</button><button className={css.dangerText} onClick={() => { setManageTargetId(item.id); setDeleteOpen(true); setManageMenuId(undefined) }}>删除需求</button></div>}</div>)}</nav>{visibleRequirements.length === 0 && <div className={css.empty}>{showArchived ? '暂无已归档需求' : '暂无进行中的需求'}</div>}</aside>
      ...
      <main className={css.editorPane}>
        <div className={css.toolbar}><div className={css.editorTitle}>{!documentsOpen && <button className={css.iconButton} aria-label="展开需求栏" title="展开需求栏" onClick={() => setDocumentsOpen(true)}>‹</button>}<strong>规范正文</strong></div><div><span className={draft !== selected?.version.markdown ? css.unsaved : css.editorMeta}>{draft !== selected?.version.markdown ? '有未保存修改' : '已保存'} · {draft.split('\n').length} 行</span><input className={css.summaryInput} required aria-label="本次修改说明（必填）" value={summary} onChange={event => setSummary(event.target.value)} placeholder="本次修改说明（必填）"/><button className={css.saveButton} disabled={busy || !selected || draft === selected.version.markdown || !summary.trim()} onClick={() => void save()}>保存新版本</button>{!reviewOpen && <button className={css.iconButton} aria-label="展开协作栏" title="展开协作栏" onClick={() => setReviewOpen(true)}>›</button>}</div></div>
        ...
        {!selected ? <div className={css.emptyMain}><button onClick={() => { setCreateOpen(true); void api.reviewWorkspaces().then(setReviewWorkspaces, error => setNotice(error instanceof Error ? error.message : String(error))) }}>输入原始需求</button></div> : <div className={css.editorBody}><RichMarkdownEditor key={`${selected.id}:${selected.currentCommit}`} markdown={draft} onChange={setDraft} onSelection={quote => setDraftSelection(quote ? { quote, prefix: '', suffix: '' } : undefined)}/></div>}
      </main>
      ...
      <aside className={`${css.review} ${reviewOpen ? '' : css.paneClosed}`}>
        <div className={css.tabbar}><div className={css.tabScroller}>{(['review', 'discussion', ...(tabCounts.items > 0 || sideTab === 'items' ? ['items' as const] : []), 'ready', ...(tabCounts.patches > 0 || sideTab === 'patches' ? ['patches' as const] : []), 'decisions', 'versions'] as SideTab[]).map(renderSideTab)}</div><button className={`${css.iconButton} ${css.collapseRight}`} aria-label="收起协作栏" title="收起协作栏" onClick={() => setReviewOpen(false)}>›</button></div>
        <div className={css.threadList}>{selected && sideTab === 'review' && <ReviewPanel requirement={selected} participant={participant} act={act} api={api}/>} {selected && sideTab === 'discussion' && <DiscussionPanel requirement={selected} selection={selection} text={commentText} setText={setCommentText} cancel={() => setSelection(undefined)} submit={addComment} participant={participant} act={act} api={api}/>} {selected && sideTab === 'patches' && <PatchPanel requirement={selected} act={act}/>} {selected && sideTab === 'decisions' && <DecisionPanel requirement={selected} act={act}/>} {selected && sideTab === 'items' && <MyItemsPanel items={currentMyItems} open={item => { setSideTab(item.target.tab); setMobilePane('collab'); window.setTimeout(() => document.getElementById(`spec-object-${item.target.objectId}`)?.scrollIntoView({ block: 'center' }), 100) }}/>} {selected && sideTab === 'versions' && <VersionPanel requirement={selected} api={api}/>} {selected && sideTab === 'ready' && <ReadyPanel requirement={selected} act={act}/>}</div>
      </aside>
    </div>
    <footer className={css.footer}><span>Git + Markdown 正式版本 · AI 只提议不确认</span><span>{selected ? `${selected.actionItems.filter(item => item.status === 'pending').length} 我的/角色事项 · ${selected.aiRuns.at(-1)?.status ?? '无 AI 任务'}` : '严格止于 Ready Spec'}</span></footer>
...
const reviewKindLabel: Record<ReviewKind, string> = { 'product-first': '产品初审', 'product-second': '产品二审', 'engineering-precheck': '研发预审', 'change-review': '变更审核' }
const statusLabel: Record<string, string> = { queued: '排队中', running: '分析中', completed: '已完成', failed: '失败', open: '待处理', 'joint-review': '待产研共审', 'non-blocking-verify': '非阻塞待验证', resolved: '已解决', invalidated: '已失效', pending: '待确认', accepted: '已接受', rejected: '已拒绝', stale: '已过期', blocking: '阻塞', major: '重要', minor: '一般' }
...
function ReviewPanel({ requirement, act, api }: { requirement: RequirementView; participant: ParticipantSnapshot; act: Act; api: SpecApi }) {
  const pendingCount = requirement.reviewItems.filter(item => !['resolved', 'invalidated'].includes(item.status)).length
  return <>
    <div className={css.panelSummary}><div><strong>{pendingCount} 个待处理问题</strong><span>{requirement.reviewItems.filter(item => item.severity === 'blocking' && !['resolved', 'invalidated'].includes(item.status)).length} 个阻塞 · {requirement.reviewItems.length - pendingCount} 个已结束</span></div></div>
    {requirement.aiRuns.length > 0 && <details className={css.runHistory}><summary>智能审核记录（{requirement.aiRuns.length}）</summary>{requirement.aiRuns.slice().reverse().map(run => <div className={css.run} key={run.id}><div><strong>{reviewKindLabel[run.kind]}</strong><span>{statusLabel[run.status] ?? run.status}{run.sessionId ? ` · ${run.sessionId.slice(0, 8)}` : ''}</span></div>{run.maturitySummary && <MarkdownBody text={run.maturitySummary}/>} {run.error && <p className={css.chatError}>{run.error}</p>}{run.sessionId && <AiConversation api={api} requirementId={requirement.id} sessionId={run.sessionId}/>}</div>)}</details>}
    {requirement.reviewItems.slice().reverse().map(item => <ReviewItemCard key={item.id} item={item} requirementId={requirement.id} act={act}/>)}
  </>
}
function ReviewItemCard({ item, requirementId, act }: { item: RequirementView['reviewItems'][number]; requirementId: string; act: Act }) {
  const ended = ['resolved', 'invalidated'].includes(item.status)
  const content = <div className={css.reviewCardBody}><div className={css.badges}><b>{statusLabel[item.severity] ?? item.severity}</b><span>{item.epistemicStatus}</span></div><div className={css.reviewField}><span>影响</span><MarkdownBody text={item.impact}/></div><div className={`${css.reviewField} ${css.questionField}`}><span>待回答问题</span><MarkdownBody text={item.question}/></div><details className={css.evidence}><summary>查看依据 · {item.evidence.length} 条</summary>{item.evidence.length === 0 ? <p>暂无可访问证据，按 TO_VERIFY 处理。</p> : item.evidence.map((evidence, index) => <div key={`${evidence.source}-${index}`}><code>{evidence.source}{evidence.version ? `@${evidence.version}` : ''}</code><MarkdownBody text={evidence.statement}/></div>)}</details>{!item.response && item.status === 'open' && <ReviewResponseForm requirementId={requirementId} reviewItemId={item.id} reviewKind={item.reviewKind} {...(item.recommendedOptions ? { options: item.recommendedOptions } : {})} act={act}/>} {item.response && <div className={css.humanResponse}><strong>{item.response.participant.nickname} 的回复</strong><MarkdownBody text={item.response.body}/></div>}</div>
  return <details id={`spec-object-${item.id}`} className={`${css.reviewDetails} ${ended ? css.endedReview : css.pendingReview}`} open={!ended}><summary><span>{item.statement}</span><b>{statusLabel[item.status] ?? item.status}</b></summary>{content}</details>
}
const dispositionLabel = { context: '补充业务语境或规则', evidence: '提供数据、案例或链接', accept: '接受 AI 建议', 'accept-modified': '修改后接受', reject: '不采纳并说明理由', 'to-verify': '当前无法回答，转待验证', 'joint-review': '转入产研共审' } as const
function ReviewResponseForm({ requirementId, reviewItemId, reviewKind, options, act }: { requirementId: string; reviewItemId: string; reviewKind: ReviewKind; options?: string[]; act: Act }) {
  ...
  return <>{recommendations.length >= 3 ? <div className={css.quickOptions}><span>业务决策选项</span>{recommendations.slice(0, 5).map((option, index) => <button key={`${index}-${option}`} onClick={() => choose(option)}>{index + 1}. {option}</button>)}</div> : <div className={css.missingOptions}><span>此历史问题暂无基于业务上下文生成的推荐选项。</span><button onClick={() => void act(actor => ({ kind: 'review.request', participant: actor, requirementId, reviewKind }))}>重新智能审核生成</button></div>}{open ? <div className={css.responseForm}><label>处理方式（必填）<select required value={disposition} onChange={event => setDisposition(event.target.value as keyof typeof dispositionLabel)}>{Object.entries(dispositionLabel).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><label>回复内容（必填）<textarea required autoFocus value={body} onChange={event => setBody(event.target.value)} placeholder={disposition === 'evidence' ? '粘贴数据、案例或来源链接，并说明它支持什么结论' : '说明你的判断、规则或取舍'}/></label><div className={css.cardActions}><button onClick={() => setOpen(false)}>取消</button><button className={css.execute} disabled={!body.trim()} onClick={() => void submit()}>提交回复</button></div></div> : <div className={css.cardActions}><button onClick={() => setOpen(true)}>自行输入</button><button onClick={() => void act(actor => ({ kind: 'review.respond', participant: actor, requirementId, reviewItemId, disposition: 'joint-review', body: '转产研共同决策' }))}>转产研共审</button></div>}</>
}
...
function PatchPanel({ requirement, act }: { requirement: RequirementView; act: Act }) {
  ...
  return <>{requirement.patches.slice().reverse().map(patch => {
    const chinese = /[\u3400-\u9fff]/.test(`${patch.summary}\n${patch.proposedMarkdown}`)
    const commitSummary = summaries[patch.id] ?? `接受中文候选 Patch：${patch.summary}`
    return <article id={`spec-object-${patch.id}`} className={css.reviewCard} key={patch.id}>
      <div className={css.badges}><b>{statusLabel[patch.status] ?? patch.status}</b><span>{reviewKindLabel[patch.reviewKind]}</span><span>{patch.baseCommit.slice(0, 8)}</span></div>
      <MarkdownBody text={patch.summary} className={css.reviewTitle}/>
      {!chinese && <div className={css.legacyWarning}><strong>旧版英文候选</strong><p>这份修改建议在中文约束上线前生成，已禁止接受。请重新运行中文审核。</p></div>}
      <details className={css.patchDetails}><summary>预览候选规范</summary><MarkdownBody text={patch.proposedMarkdown}/></details>
      {patch.status === 'pending' && (chinese ? <div className={css.patchAccept}><label>版本提交说明（必填）<input required value={commitSummary} onChange={event => setSummaries(current => ({ ...current, [patch.id]: event.target.value }))}/></label><div className={css.cardActions}><button disabled={submitting === patch.id || !commitSummary.trim()} onClick={async () => { setSubmitting(patch.id); try { await act(actor => ({ kind: 'patch.accept', participant: actor, requirementId: requirement.id, patchId: patch.id, summary: commitSummary })) } finally { setSubmitting(undefined) } }}>{submitting === patch.id ? '正在接受…' : '审核并接受'}</button><button disabled={submitting === patch.id} onClick={() => void act(actor => ({ kind: 'patch.reject', participant: actor, requirementId: requirement.id, patchId: patch.id }))}>拒绝修改</button></div></div> : <div className={css.cardActions}><button onClick={() => void act(actor => ({ kind: 'review.request', participant: actor, requirementId: requirement.id, reviewKind: patch.reviewKind }))}>重新生成中文建议</button><button onClick={() => void act(actor => ({ kind: 'patch.reject', participant: actor, requirementId: requirement.id, patchId: patch.id }))}>废弃旧建议</button></div>)}
    </article>
  })}</>
}
...
function ReadyPanel({ requirement, act }: { requirement: RequirementView; act: Act }) {
  ...
  return <><div className={css.readyHead}><h3>Ready 质量门</h3><span>{passed} / {requirement.readiness.length} 已通过</span></div><div className={css.checklist}>{requirement.readiness.map(check => <section className={`${css.checkItem} ${check.passed ? css.checkPassed : css.checkBlocked}`} key={check.key}><span className={css.checkIcon} aria-hidden="true">{check.passed ? '✓' : '!'}</span><div><strong>{readinessLabel[check.key] ?? check.key}</strong><small>{check.passed ? '已通过' : '阻塞'}</small>{check.reasons.map(reason => <MarkdownBody key={reason} text={reason}/>)}</div></section>)}</div>{requirement.readyPackage && <section><strong>Ready Spec · {requirement.readyPackage.packageHash.slice(0, 12)}</strong><MarkdownBody text={requirement.readyPackage.markdown}/></section>}{requirement.stage === 'ready' && <section className={css.downstreamReturn}><button onClick={() => setReturnOpen(value => !value)}>{returnOpen ? '取消返回问题' : '返回下游发现的问题'}</button>{returnOpen && <div><label>问题摘要（必填）<textarea required value={form.summary} onChange={event => setForm({ ...form, summary: event.target.value })} placeholder="描述下游发现的产品语义或验收问题"/></label><label>受影响章节（与 AC 至少填写一项）<input value={form.sections} onChange={event => setForm({ ...form, sections: event.target.value })} placeholder="业务术语与规则"/></label><label>受影响 AC IDs（与章节至少填写一项）<input value={form.acceptanceIds} onChange={event => setForm({ ...form, acceptanceIds: event.target.value })} placeholder="AC-RETRY-001"/></label><label>证据来源（选填）<input value={form.source} onChange={event => setForm({ ...form, source: event.target.value })} placeholder="测试报告、Issue 或代码位置"/></label><label>证据版本（选填）<input value={form.version} onChange={event => setForm({ ...form, version: event.target.value })} placeholder="commit SHA 或版本号"/></label><label>证据说明（选填）<textarea value={form.evidence} onChange={event => setForm({ ...form, evidence: event.target.value })}/></label><button className={css.execute} disabled={!form.summary.trim() || (!form.sections.trim() && !form.acceptanceIds.trim())} onClick={() => void submitReturn()}>局部重开受影响内容</button></div>}</section>}</>
}
const categoryLabel: Record<MyActionItem['category'], string> = { 'awaiting-my-response': '待我回复', 'awaiting-my-confirmation': '待我确认', 'initiated-by-me': '我发起的', 'possibly-mine-by-ip': '可能属于当前 IP' }
function MyItemsPanel({ items, open }: { items: MyActionItem[]; open: (item: MyActionItem) => void }) {
  const categories = ['awaiting-my-response', 'awaiting-my-confirmation', 'initiated-by-me', 'possibly-mine-by-ip'] as const
  return <div className={css.itemsPanel}>{categories.map(category => <section key={category}><h3>{categoryLabel[category]} · {items.filter(item => item.category === category).length}</h3>{items.filter(item => item.category === category).map(item => <button className={css.itemRow} key={`${category}-${item.id}`} onClick={() => open(item)}><span><b>{item.blocking ? '阻塞' : '普通'}</b> · {item.requirementTitle}</span><small>{item.type === 'reconfirm' ? '需要重新确认' : item.type === 'confirm' ? '等待确认' : '等待回复'} · {new Date(item.updatedAt).toLocaleString()}</small></button>)}{items.every(item => item.category !== category) && <p className={css.empty}>当前需求暂无事项</p>}</section>)}</div>
}
...
function IdentityDialog({ value, boundRole, onChange, close }: { value: ParticipantSnapshot; boundRole?: ParticipantRole; onChange: (value: ParticipantSnapshot) => void; close: () => void }) { return <div className={css.modal}><form onSubmit={event => { event.preventDefault(); if (value.nickname.trim().length >= 2) close() }}><h2>登记参与花名</h2><p>匿名可读。首次成功写入后，Host 会将 participant_id 与责任角色绑定；IP 不作为身份或权限依据。</p><label>花名（必填）<input required autoFocus value={value.nickname} onChange={event => onChange({ ...value, nickname: event.target.value })}/></label><label>责任角色（必填）<select required disabled={boundRole !== undefined} value={boundRole ?? value.role} onChange={event => onChange({ ...value, role: event.target.value as ParticipantRole })}><option value="product">产品</option><option value="engineering">研发</option></select></label>{boundRole && <small>该身份已由 Host 绑定为{boundRole === 'product' ? '产品' : '研发'}角色。</small>}<button type="submit" disabled={value.nickname.trim().length < 2}>保存身份</button></form></div> }
...
function WorkspaceDialog({ value, setValue, workspaces, close, submit, busy }: { value: string; setValue: (value: string) => void; workspaces: ReviewWorkspaceSummary[]; close: () => void; submit: () => void; busy: boolean }) { return <div className={css.modal}><form onSubmit={event => { event.preventDefault(); submit() }}><h2>绑定项目工作区</h2><label>项目工作区（必填）<select required autoFocus value={value} onChange={event => setValue(event.target.value)}><option value="" disabled>{workspaces.length > 0 ? '选择 DSH 项目工作区' : '暂无可用的 DSH 工作区'}</option>{workspaces.map(workspace => <option key={workspace.workspaceId} value={workspace.workspaceId}>{workspace.title} · {workspace.path}</option>)}</select></label><div><button type="button" onClick={close}>取消</button><button type="submit" disabled={busy || !value}>确认绑定</button></div></form></div> }
function CreateDialog({ value, setValue, workspaces, close, submit, busy }: { value: { title: string; rawRequirement: string; sources: string; workspaceId: string }; setValue: (value: { title: string; rawRequirement: string; sources: string; workspaceId: string }) => void; workspaces: ReviewWorkspaceSummary[]; close: () => void; submit: () => void; busy: boolean }) { return <div className={css.modal}><form onSubmit={event => { event.preventDefault(); submit() }}><h2>输入原始需求</h2><label>需求标题（必填）<input required value={value.title} onChange={event => setValue({ ...value, title: event.target.value })}/></label><label>项目工作区（必填）<select required value={value.workspaceId} onChange={event => setValue({ ...value, workspaceId: event.target.value })}><option value="" disabled>{workspaces.length > 0 ? '选择 DSH 项目工作区' : '暂无可用的 DSH 工作区'}</option>{workspaces.map(workspace => <option key={workspace.workspaceId} value={workspace.workspaceId}>{workspace.title} · {workspace.path}</option>)}</select></label><label>自然语言需求或已有 Markdown（必填）<textarea required value={value.rawRequirement} onChange={event => setValue({ ...value, rawRequirement: event.target.value })} placeholder="描述用户问题、期望结果、已有方案和已知限制"/></label><label>来源、链接或代码范围（选填，每行一个）<textarea value={value.sources} onChange={event => setValue({ ...value, sources: event.target.value })} placeholder="https://...&#10;历史 Decision ...&#10;workspace/repository scope"/></label><div><button type="button" onClick={close}>取消</button><button type="submit" disabled={busy || !value.title.trim() || !value.rawRequirement.trim() || !value.workspaceId}>创建并固定输入版本</button></div></form></div> }
