...
export function SpecWorkbench({ api }: { api: SpecApi }) {
  ...
  return <div className={css.workbench}>
    ...
    <div className={css.mobileViewSwitch}><button className={mobilePane === 'spec' ? css.activeMode : ''} onClick={() => setMobilePane('spec')}>规范正文</button><button className={mobilePane === 'collab' ? css.activeMode : ''} onClick={() => setMobilePane('collab')}>协作处理{selected ? ` · ${currentPendingActions(selected).length}` : ''}</button></div>
    ...
    <footer className={css.footer}><span>Git + Markdown 正式版本 · AI 只提议不确认</span><span>{selected ? `${currentPendingActions(selected).length} 当前版本事项 · ${selected.aiRuns.at(-1)?.status ?? '无 AI 任务'}` : '严格止于 Ready Spec'}</span></footer>
